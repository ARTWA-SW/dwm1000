...
// GPIOs
typedef enum {
  DWT_GxP0 = 0x00000001UL, /* GPIO0 Only changed if the GxM0 mask bit has a value of 1 for the write operation*/
  DWT_GxP1 = 0x00000002UL, /* GPIO1.*/
  DWT_GxP2 = 0x00000004UL, /* GPIO2.*/
  DWT_GxP3 = 0x00000008UL, /* GPIO3.*/
  DWT_GxP4 = 0x00000100UL, /* GPIO4.*/
  DWT_GxP5 = 0x00000200UL, /* GPIO5.*/
  DWT_GxP6 = 0x00000400UL, /* GPIO6.*/
  DWT_GxP7 = 0x00000800UL, /* GPIO7.*/
  DWT_GxP8 = 0x00010000UL  /* GPIO8 */
} dwt_gpio_pin_e;

/* Mask values for GPIO pins on DW1000 */
typedef enum {
  DWT_GxM0 = 0x00000010UL, /* Mask for GPIO0 */
  DWT_GxM1 = 0x00000020UL, /* Mask for GPIO1 */
  DWT_GxM2 = 0x00000040UL, /* Mask for GPIO2 */
  DWT_GxM3 = 0x00000080UL, /* Mask for GPIO3 */
  DWT_GxM4 = 0x00001000UL, /* Mask for GPIO4 */
  DWT_GxM5 = 0x00002000UL, /* Mask for GPIO5 */
  DWT_GxM6 = 0x00004000UL, /* Mask for GPIO6 */
  DWT_GxM7 = 0x00008000UL, /* Mask for GPIO7 */
  DWT_GxM8 = 0x00100000UL  /* Mask for GPIO8 */
} dwt_gpio_mask_e;
 ...
 * @warning also changes sign so a positive value means the local RX clock is running slower than the remote TX device.
 */
 ...
 *
 * @note 1. When DW1000 is powered on this function needs to be run before dwt_configuresleep,
 *       also the SPI frequency has to be < 3MHz
 * @note 2. It reads and applies LDO tune and crystal trim values from OTP memory
 * @note 3. If accurate RX timestamping is needed microcode/LDE must be loaded
 *
 ...
 *                         NB: this is the length of TX message (including the 2 byte CRC) - max is 1023
 ...
 *                      NB: standard PHR mode allows up to 127 bytes
 *                      if > 127 is programmed, DWT_PHRMODE_EXT needs to be set in the phrMode configuration
 *                      see dwt_configure function
 ...
 * @note - dwt_configuresleep() to configure long sleep phase. "mode" parameter should at least have DWT_PRESRV_SLEEP,
 *          DWT_CONFIG and DWT_RX_EN set and "wake" parameter should at least have both DWT_WAKE_SLPCNT and DWT_SLP_EN set.
 * @note - dwt_calibratesleepcnt() and dwt_configuresleepcnt() to define the "long sleep" phase duration.
 * @note - dwt_setsnoozetime() to define the "short sleep" phase duration.
 * @note - dwt_setpreambledetecttimeout() to define the reception phases duration.
 * @note - dwt_setinterrupt() to activate RX good frame interrupt (DWT_INT_RFCG) only.
 *
 * @note
 ...
 *
 * @returns value is 1 if the IRQS bit is set and 0 otherwise
 */
 ...
 *
 * @warning To correctly read the temperature this read should be done with xtal clock
 ...
 *
 * @param target_count the PG count target to reach in order to correct the bandwidth
 *
 ...
 *
 * @param channel the channel at which compensation of power level will be applied: 2 or 5
 * @param ref_powerreg the TX_POWER register value recorded when reference measurements were made
 * @param delta_temp the difference between current ambient temperature (raw value units)
 ...
 *
 * @param pgdly the PG_DELAY to set (to control bandwidth), and to find the corresponding count value for
 * None
 *
 ...
 *
 * @note 1. Firstly we create a header (the first byte is a header byte)
 * @note  a. check if sub index is used, if subindexing is used - set bit-6 to 1 to signify that the sub-index address follows the register index byte
 * @note  b. set bit-7 (or with 0x80) for write operation
 * @note  c. if extended sub address index is used (i.e. if index > 127) set bit-7 of the first sub-index byte following the first header byte
 * @note 2. Write the header followed by the data bytes to the DW1000 device
 *
 ...
 * @note  c. if extended sub address index is used (i.e. if index > 127) set bit-7 of the first sub-index byte following the first header byte
 * @note 2. Write the header followed by the data bytes to the DW1000 device
 * @note 3. Store the read data in the input buffer
 *
 * @param recordNumber ID of register file or buffer being accessed
 * @param index byte index into register file or buffer being accessed
